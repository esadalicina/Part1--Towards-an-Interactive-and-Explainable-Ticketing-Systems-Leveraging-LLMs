...
model_list = [
    ...
    create_rnn_model(vocab_size=X_train_tf_resampled.shape[1], output_dim=len(np.unique(train_labels_resampled)), input_length=X_train_tf_resampled.shape[1])
]
...
for model in model_list:
    ...
    model.fit(to_dense(X_train_tf_resampled), train_labels_resampled, epochs=10, batch_size=32, verbose=0)
    final_accuracy = model.evaluate(to_dense(X_test_tf), test_labels, verbose=0)[1]
    y_pred = np.argmax(model.predict(to_dense(X_test_tf)), axis=-1)
    ...
    model.fit(to_dense(train_embeddings_resampled), train_labels_resampled_w2v, epochs=10, batch_size=32, verbose=0)
    final_accuracy_w2v = model.evaluate(to_dense(test_embeddings), test_labels, verbose=0)[1]
